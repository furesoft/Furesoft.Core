...
namespace Furesoft.Core.CodeDom.Parsing
{
    ...
    public class Parser : IDisposable
    {
        ...
        public Parser(CodeUnit codeUnit, ParseFlags flags, bool isGenerated)
        {
            ...
            if (codeUnit.IsFile)
            {
                var fileStream = new FileStream(codeUnit.FileName, FileMode.Open, FileAccess.Read, FileShare.Read);
                var bom = new byte[3];
        ...
        {
            var nextToLast = GetUnused(Unused.Count - 1 + offset);
            if (nextToLast is Token && ((Token)nextToLast).IsIdentifier)
                return true;
            if (nextToLast is UnusedCodeObject)
            {
                var codeObject = ((UnusedCodeObject)nextToLast).CodeObject;
                ...
                {
                    var right = ((Dot)codeObject).SkipPrefixes();
        ...
        public CodeObject ProcessToken(CodeObject parent, ParseFlags flags)
        {
                    ...
                    case TokenType.Symbol:
                        {
                            ...
                            if (_tokenMap.TryGetValue(TokenText, out list))
                            {
                                ...
                                foreach (var parsePoint in list)
                                {
                            ...
                            if (obj == null)
                            {
                                ...
                                if (TokenText == Statement.ParseTokenTerminator)
                                {
                                    ...
                                    NextToken();
                                    var comments = LastToken.TrailingComments;
                                ...
                                else
                                {
                                    ...
                                    {
                                        var number = int.Parse(PeekNextTokenText());
                                        if (number > 0)
                                        {
                                            NextToken(); // Skip past '`'
            ...
            catch (Exception ex)
            {
                ...
                var badTokenText = "'" + (tokenText != null && tokenText.Length > MaxBadTokenDisplayLength ? tokenText.Substring(0, MaxBadTokenDisplayLength) + "..." : tokenText) + "'";
                var message = CodeUnit.LogException(ex, "parsing token " + badTokenText);
        ...
        protected void ProcessTrailingComments()
        {
            ...
            do
            {
                ...
                var comment = (CommentBase)ProcessToken(null);
                if (comment != null)
        ...
        {
            var next = NextToken(includeComments);
            if (next == null)
                return null;
        ...
        public void MoveCommentsToUnused()
        {
            ...
            {
                foreach (var commentBase in LastToken.TrailingComments)
        ...
        protected Token ParseNextToken()
        {
            ...
            var whitespace = _line.Substring(_start, _pos - _start);

            // Mark start of token
            _start = _pos;
            var tokenStart = _start;  // Preserve start in case there are multiple lines (such as a block comment)
            _previous = "";
            var tokenStartLine = LineNumber;
            var newLines = LineNumber - _startLine;
            var skipSpace = false;
            var leaveNewLine = false;
            var wasEscaped = _wasEscaped;
            var hasUnicodeEscapeSequence = false;
            TokenType tokenType;
            ...
            else
            {
                ...
                else if (char.IsDigit(_ch) || (_ch == '.' && char.IsDigit(_pk)))
                {
                    ...
                    else
                    {
                        ...
                        // Handle exponent
                        var pk2 = ((_pos < _length - 2) ? _line[_pos + 2] : '\0');
        ...
        protected internal void NextChar()
        {
            ...
            if (++_pos >= _length)
            {
                ...
                if (_line.Length > 0)
                {
                    ...
                    else if (_line[0] == '\t')
                    {
                        ...
                        var expanded = new StringBuilder(_line.Length * 2);
                ...
                if (_inDocComment)
                {
                    ...
                    if (_inBlockDocComment)
                    {
                        if (_blockDocCommentPrefix != null)
                        {
                            ...
                            var prefixLength = _blockDocCommentPrefix.Length;
                            ...
                            else
                            {
                                ...
                                var whitespace = StringUtil.CharCount(_line, ' ', 0);
            ...
            if (_inDocCommentCodeContent)
            {
                var terminatorLength = _docCommentCodeContentTerminator.Length;
        ...
        public string GetTokenToEOL()
        {
            ...
            var result = TokenText + _line.Substring(_start, _pos - _start);
            NextChar();
            NextToken();
            return result;
        }
        ...
        public string GetToDelimiter(char delimiter)
        {
            ...
            if (_pos < 0)
                _pos = _length - 1;  // Stop just short of the EOL
            var result = _line.Substring(_start, _pos - _start);
            --_pos;
            NextChar();
            NextToken();
            return result;
        ...
        public Expression ParseCodeExpressionUntil(string delimiter, CodeObject parent)
        {
            ...
            var expression = Expression.Parse(this, parent, true);

            // Clear the special termination mode, and fix the Token variables if necessary
            var stillInContent = _inDocCommentCodeContent;
            ...
            if (stillInContent && !delimiter.StartsWith(TokenText))
            {
                ...
                {
                    var badText = Token.LeadingWhitespace + TokenText + GetToDelimiter(delimiter[0]);
                    var unrecognized = new Unrecognized(false, _inDocComment, expression, new UnresolvedRef(badText, Token));
        ...
        {
            var parsePoint = new ParsePoint { ContextTypes = contextTypes, Priority = priority, Callback = callback };
            List<ParsePoint> list;
        ...
        private class ParsePoint
        {
            ...
            public bool IsContextValid(Parser parser, CodeObject parent, ParseFlags flags)
            {
                ...
                if (ContextTypes != null && ContextTypes.Length > 0 && parent != null)
                {
                    ...
                    var relaxedMode = parser.InDocComment;
                    validContext = false;
