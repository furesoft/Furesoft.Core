    ...
    public static class StringUtil
    {
        ...
        public static int CharCount(string thisStr, char ch, int startIndex)
        {
            ...
            if (thisStr != null)
            {
                ...
                for (var i = startIndex; i < length && thisStr[i] == ch; ++i)
                    ++count;
            }
            return count;
        }
        ...
        public static string ToString(IEnumerable thisCollection, string separator)
        {
            ...
            {
                foreach (var obj in thisCollection)
                {
                    if (obj != null)
        ...
        {
            var results = defaultValues;
            if (!string.IsNullOrEmpty(thisStr))
            {
                try
                {
                    var fields = Enumerable.Select<string, T>(thisStr.Split(delim), delegate (string text) { return Parse<T>(text); });
        ...
        public static string ExpandEnvironmentMacros(string value)
        {
            if (!string.IsNullOrEmpty(value))
            {
                ...
                if (matches.Count > 0)
                {
                    ...
                    {
                        var match = matches[i];
                        var skippedLength = match.Index - index;
                    ...
                    if (len > 0)
                    {
                        try
                        {
                            if (CheckHex(source, i + 2, len))
                            {
                                ...
                                var hex = source.Substring(i + 2, len);
                                if (len == 4)
                                {
                                    var u16 = ushort.Parse(hex, NumberStyles.HexNumber);
                                    result.Append((char)u16);
                                }
                                else
                                {
                                    var u32 = int.Parse(hex, NumberStyles.HexNumber);
