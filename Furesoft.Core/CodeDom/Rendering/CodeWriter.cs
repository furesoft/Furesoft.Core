...
namespace Furesoft.Core.CodeDom.Rendering
{
    ...
    public class CodeWriter : IDisposable
    {
        ...
        {
            var indentState = _indentStateStack.Peek();
        ...
        public void BeginIndentOnNewLineRelativeToLastIndent(CodeObject codeObject, CodeObject lastCodeObject)
        {
            ...
            var indentState = _lastPoppedIndentState;
        ...
        {
            var indentState = _indentStateStack.Peek();
            var newOffset = (offset < 0 ? indentState.IndentOffsetOnNewLine + offset : offset);
        ...
        {
            var alignmentState = _alignmentStateStack.Pop();
        ...
        {
            foreach (var alignmentState in _alignmentStateStack)
            {
        ...
        {
            foreach (var indentState in _indentStateStack)
            {
        ...
        public void Write(string text)
        {
            ...
            if (_isEmptyLine)
            {
                ...
                var indentOffset = _indentStateStack.Peek().IndentOffset;
                if (_textWriter != null)
                {
                    var indentString = (UseTabs ? new string('\t', indentOffset / CodeObject.TabSize) + new string(' ', indentOffset % CodeObject.TabSize)
                ...
                {
                    var leadingSpaces = StringUtil.CharCount(text, ' ', 0);
                    var tabs = leadingSpaces / CodeObject.TabSize;
                    _textWriter.Write(new string('\t', tabs));
                    var offset = tabs * CodeObject.TabSize;
            ...
            for (i = 0; i < text.Length; ++i)
            {
                ...
                if (text[i] > 0xff && EscapeUnicode)
                {
                    ...
                    {
                        var u32 = char.ConvertToUtf32(text, i);
        ...
        public void WriteList<T>(IEnumerable<T> enumerable, CodeObject.RenderFlags flags, CodeObject parent, int[] columnWidths) where T : CodeObject
        {
            ...
            var increaseIndent = !flags.HasFlag(CodeObject.RenderFlags.NoIncreaseIndent);
            if (increaseIndent)
            ...
            while (hasMore)
            {
                ...
                if (codeObject != null)
                {
                    ...
                    var passFlags = flags | CodeObject.RenderFlags.NoEOLComments | CodeObject.RenderFlags.NoPostAnnotations | CodeObject.RenderFlags.SuppressNewLine
                    ...
                    if (hasMore)
                    {
                        ...
                        var isLastOnLine = (nextObject != null && nextObject.IsFirstOnLine);
                        if (!isLastOnLine)
                    ...
                    else
                    {
                        ...
                        var hasCloseBraceOnSameLine = (parent is Initializer && !((Initializer)parent).IsEndFirstOnLine);
        ...
        public void WritePendingEOLComment(Comment comment, CodeObject.RenderFlags flags)
        {
            ...
            var parentOffset = _indentStateStack.Peek().ParentOffset;
        ...
        protected void FlushPendingEOLComments(bool isEndOfLine)
        {
            if (!_flushingEOLComments)  // Prevent re-entry
            {
                ...
                foreach (var eolComment in _pendingEOLComments)
                {
                    ...
                    var comment = eolComment.Comment;
        ...
        private void WritePadding(CodeObject codeObject, int[] columnWidths, int column)
        {
            ...
            {
                var paddingLength = columnWidths[column] - codeObject.AsTextLength(CodeObject.RenderFlags.LengthFlags, AlignmentStateStack);
                if (paddingLength > 0)
                {
                    if (_textWriter != null)
