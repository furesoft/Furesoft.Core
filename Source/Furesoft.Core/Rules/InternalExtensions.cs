...
internal static class InternalExtensions
{
    ...
    {
        return !rule.Configuration.Skip && rule.Configuration.Constraint.Invoke2(rule.Model);
    }

    public static bool Invoke2<T>(this Predicate<T> predicate, T model)
    {
        return predicate == null || predicate(model);
    }
    ...
    {
        return ruleEngineConfiguration.Terminate != null && ruleEngineConfiguration.Terminate.Value;
    }
    ...
    public static IGeneralRule<T> GetGlobalExceptionHandler<T>(this IEnumerable<IGeneralRule<T>> rules)
        where T : class, new()
    {
