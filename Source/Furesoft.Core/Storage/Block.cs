...
public class Block : IBlock
{
    ...
    {
        if (_isDisposed) throw new ObjectDisposedException("Block");

        // Validate field number
        if (field < 0) throw new IndexOutOfRangeException();
        if (field >= _storage.BlockHeaderSize / 8) throw new ArgumentException("Invalid field: " + field);
    ...
    public void SetHeader(int field, long value)
    {
        ...
        if (field < _cachedHeaderValue.Length) _cachedHeaderValue[field] = value;
    ...
    public void Read(byte[] dest, int destOffset, int srcOffset, int count)
    {
        ...
        if (copyFromFirstSector)
        {
            ...
            Buffer.BlockCopy(_firstSector
                , _storage.BlockHeaderSize + srcOffset
                , dest
                , destOffset
                , tobeCopied);
        ...
        while (dataCopied < count)
        {
            ...
            if (thisRead == 0) throw new EndOfStreamException();
            dataCopied += thisRead;
        }
    }
    ...
    public void Write(byte[] src, int srcOffset, int dstOffset, int count)
    {
        ...
        if (_storage.BlockHeaderSize + dstOffset < _storage.DiskSectorSize)
        {
            ...
            Buffer.BlockCopy(src
                , srcOffset
                , _firstSector
                , _storage.BlockHeaderSize + dstOffset
                , thisWrite);
            _isFirstSectorDirty = true;
        }
