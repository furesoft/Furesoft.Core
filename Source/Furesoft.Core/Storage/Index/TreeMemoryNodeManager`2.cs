...
public class TreeMemoryNodeManager<K, V> : ITreeNodeManager<K, V>
{
    ...
    {
        if (!_nodes.ContainsKey(id)) throw new ArgumentException("Node not found by id: " + id);
    ...
    {
        if (target == RootNode) RootNode = null;
        if (_nodes.ContainsKey(target.Id)) _nodes.Remove(target.Id);
    }
