...
public class TreeNode<K, V>
{
        ...
        , IEnumerable<uint> childrenIds = null)
    {
        ...
        if (childrenIds != null) this.childrenIds.AddRange(childrenIds);
    }
    ...
    public K MaxKey => entries[entries.Count - 1].Item1;
    ...
    {
        // Validate argument
        if (false == removeAt >= 0 && removeAt < entries.Count) throw new ArgumentOutOfRangeException();
        ...
        else
        {
            ...
            // REplace it
            entries[removeAt] = replacementEntry;
            nodeManager.MarkAsChanged(this);
    ...
    public int IndexInParent()
    {
        ...
        if (parent == null) throw new("IndexInParent fails to find parent node of " + id);
    ...
    public void Split(out TreeNode<K, V> outLeftNode, out TreeNode<K, V> outRightNode)
    {
        ...
        if (false == IsLeaf) childrenIds.RemoveRange(halfCount + 1);
        ...
        if (parent == null)
        {
            parent = nodeManager.CreateNewRoot(middleEntry.Item1
                , middleEntry.Item2
                , id
                , newRightNode.Id);
            ParentId = parent.Id;
        ...
        else
        {
            ...
            if (parent.IsOverflow) parent.Split(out var left, out var right);
        }
    ...
    {
        return entries.BinarySearch(new(key, default), nodeManager.EntryComparer);
    }
    ...
    {
        if (firstOccurence)
            return entries.BinarySearchFirst(new(key, default), nodeManager.EntryComparer);
        return entries.BinarySearchLast(new(key, default), nodeManager.EntryComparer);
    }
    ...
    public override string ToString()
    {
        if (IsLeaf)
        {
            var numbers = (from tuple in entries select tuple.Item1.ToString()).ToArray();
        ...
        {
            var numbers = (from tuple in entries select tuple.Item1.ToString()).ToArray();
            var ids = (from id in childrenIds select id.ToString()).ToArray();
