...
public class Tree<K, V> : IIndex<K, V>
{
    ...
    public void Insert(K key, V value)
    {
        ...
        // Duplication check
        if (insertionIndex >= 0 && false == _allowDuplicateKeys) throw new TreeKeyExistsException(key);
        ...
        if (leafNode.IsOverflow) leafNode.Split(out var left, out var right);
    ...
    public IEnumerable<Tuple<K, V>> LargerThanOrEqualTo(K key)
    {
        ...
        var node = FindNodeForIteration(key, nodeManager.RootNode, true, ref startIterationIndex);
    ...
    {
        var startIterationIndex = 0;
        var node = FindNodeForIteration(key, nodeManager.RootNode, false, ref startIterationIndex);
    ...
    private TreeNode<K, V> FindNodeForIteration(K key, TreeNode<K, V> node, bool moveLeft, ref int startIterationIndex)
    {
        ...
        if (binarySearchResult >= 0)
        {
            ...
            return FindNodeForIteration(key, node.GetChildNode(moveLeft ? binarySearchResult : binarySearchResult + 1),
                moveLeft, ref startIterationIndex);
        }
