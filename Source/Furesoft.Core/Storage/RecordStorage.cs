...
public class RecordStorage : IRecordStorage
{
    ...
    public RecordStorage(IBlockStorage storage)
    {
        ...
        if (storage.BlockHeaderSize < 48) throw new ArgumentException("Record storage needs at least 48 header bytes");
    }
    ...
    public virtual byte[] Find(uint recordId)
    {
        ...
        if (block.GetHeader(KIsDeleted) == 1L) return null;

        // If this block is a child block then also ignore it
        if (block.GetHeader(KPreviousBlockId) != 0L) return null;
        ...
        while (true)
        {
            ...
            using (currentBlock)
            {
                ...
                currentBlock.Read(data, bytesRead, 0, (int) thisBlockContentLength);
                ...
                if (nextBlockId == 0) return data;
            }

            currentBlock = storage.Find(nextBlockId);
            if (currentBlock == null) throw new InvalidDataException("Block not found by id: " + nextBlockId);
        }
    }

    public virtual uint Create(Func<uint, byte[]> dataGenerator)
    {
        if (dataGenerator == null) throw new ArgumentException();
        ...
        if (dataTobeWritten == 0) return returnId;
        ...
        while (dataWritten < dataTobeWritten)
        {
            ...
            using (currentBlock)
            {
                ...
                var thisWrite = Math.Min(storage.BlockContentSize, dataTobeWritten - dataWritten);
            ...
            if (nextBlock != null) currentBlock = nextBlock;
        }
        ...
        try
        {
            ...
            while (written < total)
            {
                ...
                else
                {
                    ...
                    if (target == null) throw new("Failed to allocate new block");
                    blocks.Add(target);
                }
                ...
                if (written == 0) target.SetHeader(KRecordLength, total);
    ...
    private IBlock AllocateBlock()
    {
        ...
        if (!TryFindFreeBlock(out var resuableBlockId))
        {
            ...
            if (newBlock == null) throw new("Failed to create new block");
        }
        else
        {
            newBlock = storage.Find(resuableBlockId);
            if (newBlock == null) throw new InvalidDataException("Block not found by id: " + resuableBlockId);
    ...
    private void AppendUInt32ToContent(IBlock block, uint value)
    {
        ...
        if (contentLength % 4 != 0) throw new DataMisalignedException("Block content length not %4: " + contentLength);
    ...
    private uint ReadUInt32FromTrailingContent(IBlock block)
    {
        ...
        if (contentLength == 0) throw new InvalidDataException("Trying to dequeue UInt32 from an empty block");
    ...
    private void GetSpaceTrackingBlock(out IBlock lastBlock, out IBlock secondLastBlock)
    {
        ...
        try
        {
            if (blocks == null || blocks.Count == 0) throw new("Failed to find blocks of record 0");

            // Assign
            lastBlock = blocks[blocks.Count - 1];
            if (blocks.Count > 1) secondLastBlock = blocks[^2];
        }
        finally
        {
