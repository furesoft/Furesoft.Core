...
{
    private static readonly Regex rUnit = new(
        @"(\(((?<1>(10|2)\^-?\d+)(\*|\.)?)*\))?(?<2>[^\*\.\-0123456789]+)(?<3>-?\d+)?|(?<4>(10|2)\^-?\d+)(?=\*|\.)?",
    ...
    private bool TryParseExponent(Match match, out ExponentMeasureUnit u, bool skipFirstLookup = false)
    {
        ...
        foreach (Capture f in match.Groups[1].Captures) adjustment = ParseFactor(f.Value, adjustment);
