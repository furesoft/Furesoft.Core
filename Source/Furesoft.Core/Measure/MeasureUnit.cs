...
{
    private readonly ExponentMeasureUnit[] _units;
    private MeasureUnit _invert;
    private MeasureUnit _normalization;
    private FullFactor _normalizationFactor;
    ...
    {
        var measures = new Combinator(null);
        ...
        return (measures.GetResult(Context), f);
    }
    ...
    public MeasureUnit Multiply(MeasureUnit m)
    {
        return Context == null
            ? m
            : m == None
                ? this
                : Combinator.Create(Context, MeasureUnits.Concat(m.MeasureUnits));
    }
    ...
    public MeasureUnit DivideBy(MeasureUnit m)
    {
        return Context == null
            ? m.Invert()
            : m == None
                ? this
                : Combinator.Create(Context, MeasureUnits.Concat(m.Invert().MeasureUnits));
    }
    ...
    {
        if (exp == 0 || Context == null) return None;
        if (exp == 1) return this;
        if (exp == -1) return Invert();
        var c = new Combinator(null);
    ...
    public MeasureUnit Invert()
    {
        if (_invert == null)
        {
            var c = new Combinator(null);
            foreach (var m in MeasureUnits)
                if (m.AtomicMeasureUnit != None)
                    c.Add(m.AtomicMeasureUnit, -m.Exponent);
            if (_invert == null)
            {
                _invert = c.GetResult(Context);
    ...
    {
        return StandardMeasureContext.Default.DefineFundamental(abbreviation, name, autoStandardPrefix,
            normalizedPrefix);
    }
    ...
    {
        return withName ? $"{Abbreviation} ({Name})" : Abbreviation;
    }
...
#pragma warning disable 1591
    public static MeasureUnit operator /(MeasureUnit o1, MeasureUnit o2)
    {
    ...
    public static MeasureUnit operator ^(MeasureUnit o, int exp)
    {
        return o.Power(exp);
    }
