...
{
    /// <summary>
    ///     This is the "G15" format used when calling <see cref="double.ToString(string, IFormatProvider)" /> (with the
    ///     <see cref="CultureInfo.InvariantCulture" />)
    ///     to obtain a string where small rounding adjustments are erased.
    ///     <para>
    ///         The float formatting has changed with .Net Core 3.0 - see
    ...
    {
        return new(Value * q.Value, Unit * q.Unit);
    }
    ...
    /// <returns>The resulting quantity.</returns>
    public Quantity DivideBy(Quantity q)
    {
        return new(Value / q.Value, Unit / q.Unit);
    }
    ...
    {
        return new(1.0 / Value, Unit.Invert());
    }
    ...
    {
        return new(Math.Pow(Value, exp), Unit.Power(exp));
    }
    ...
    public bool CanAdd(Quantity q)
    {
        return q.CanConvertTo(Unit);
    }
    ...
    ///     Must be called only if <see cref="CanConvertTo(MeasureUnit)" /> returned true otherwise an
        ...
        if (!CanConvertTo(u))
        {
            if (u.Context != Unit.Context)
                throw new ArgumentException(
                    $"Can not convert between units in different contexts ('{Unit}' to '{u}').");
...
#pragma warning disable 1591
    public static Quantity operator /(Quantity o1, Quantity o2)
    {
        return o1.DivideBy(o2);
    }
    ...
    public static Quantity operator ^(Quantity o, int exp)
    {
        return o.Power(exp);
    }

    public static Quantity operator /(Quantity o, double v)
    {
        return new(o.Value / v, o.Unit);
    }
    ...
    public static Quantity operator /(double v, Quantity o)
    {
        return new(v / o.Value, o.Unit.Invert());
    }
    ...
    public static Quantity operator -(Quantity o)
    {
        return o.Negate();
    }
    ...
    public static bool operator ==(Quantity o1, Quantity o2)
    {
        return o1.Equals(o2);
    }
...
#pragma warning restore 1591
    ...
    {
        if (_normalized == null) _normalized = Unit == null ? "0" : ConvertTo(Unit.Normalization).ToRoundedString();
        return _normalized;
    }
    ...
    {
        return obj is Quantity q && Equals(q);
    }
    ...
    {
        return Unit.Normalization.GetHashCode() ^ ToNormalizedString().GetHashCode();
    }
    ...
    {
        return Unit.Context == other.Unit.Context
    ...
    public int CompareTo(Quantity other)
    {
        ...
        if (tU.Context == oU.Context)
        {
            ...
            if (ToNormalizedString() == other.ToNormalizedString()) return 0;
            // Same unit, we compare the Values.
            if (tU == oU) return Value.CompareTo(other.Value);
