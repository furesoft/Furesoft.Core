...
{
	/// <summary>
	///     Qualifies the abbreviation potential clash detected by
	///     <see cref="CheckValidNewAbbreviation(string, AutoStandardPrefix)" />.
	/// </summary>
    ...
    {
        if (_allUnits.ContainsKey(a)) return NewAbbreviationConflict.Exists;
    ...
    ///     <see cref="FundamentalMeasureUnit" /> but one of its
        ...
        MeasureStandardPrefix normalizedPrefix = null)
    {
        ...
        if (normalizedPrefix.Base == 2 && (autoStandardPrefix & AutoStandardPrefix.Binary) == 0)
            throw new ArgumentException(
                $"Since the normalization is {normalizedPrefix.Name}, {abbreviation} unit must support AutoStandardPrefix.Binary.",
                nameof(autoStandardPrefix));
        if (normalizedPrefix.Base == 10 && (autoStandardPrefix & AutoStandardPrefix.Metric) == 0)
            throw new ArgumentException(
                $"Since the normalization is {normalizedPrefix.Name}, {abbreviation} unit must support AutoStandardPrefix.Metric.",
                nameof(autoStandardPrefix));
    ...
    private AliasMeasureUnit RegisterAlias(string a, string n, FullFactor f, MeasureUnit d,
        AutoStandardPrefix stdPrefix)
    {
        ...
        return Register(a,
            n,
            () => new AliasMeasureUnit(this, a, n, f, d, stdPrefix),
            m =>
            {
                ...
                if (m.NormalizationFactor != d.NormalizationFactor.Multiply(f))
                    ThrowArgumentException(m,
                        $"new normalization factor '{f}' should be '{m.NormalizationFactor.DivideBy(d.NormalizationFactor)}'.");
    ...
    /// </summary>
    internal PrefixedMeasureUnit RegisterPrefixed(ExpFactor adjustment, MeasureStandardPrefix p, AtomicMeasureUnit u,
        bool? isNormalized = null)
    {
        var names = PrefixedMeasureUnit.ComputeNames(adjustment, p, u);
        return Register(names.A, names.N,
            () => new PrefixedMeasureUnit(this, names, adjustment, p, u, isNormalized ?? false), m =>
            {
    ...
    private FundamentalMeasureUnit RegisterFundamental(string abbreviation, string name, AutoStandardPrefix stdPrefix,
        bool isNormalized)
    {
        return Register(abbreviation, name,
            () => new FundamentalMeasureUnit(this, abbreviation, name, stdPrefix, isNormalized),
            m =>
            {
    ...
    private T Register<T>(string abbreviation, string name, Func<T> creator, Action<T> checker)
        where T : MeasureUnit
    {
        var m = CheckSingleRegistration(abbreviation, name, checker);
        if (m == null)
        {
            var newOne = creator();
    ...
    private T CheckSingleRegistration<T>(string abbreviation, string name, Action<T> check)
        where T : MeasureUnit
    {
        T result = null;
        if (_allUnits.TryGetValue(abbreviation, out var m))
        {
            ...
            if ((result = m as T) == null)
                ThrowArgumentException(m, $"new type '{typeof(T).Name}' differ from '{m.GetType().Name}'.");
            check?.Invoke(result);
        }
    ...
    private void CheckArgumentAbbreviation(string abbreviation, AutoStandardPrefix autoStandardPrefix)
    {
    ...
    private void CheckArgumentAutoStdPrefix(AtomicMeasureUnit m, AutoStandardPrefix stdPrefix)
    {
    ...
    private void ThrowArgumentException(MeasureUnit m, string explain)
    {
        throw new ArgumentException($"Already existing unit registration (context '{Name}') for '{m}': " + explain);
    }
}