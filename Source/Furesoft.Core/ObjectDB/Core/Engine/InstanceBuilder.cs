...
internal sealed class InstanceBuilder : IInstanceBuilder
{
    ...
    private static object CheckIfOid(AbstractObjectInfo objectInfo, int odbTypeId)
    {
        ...
        {
            if (objectInfo.GetObject() is long num)
            {
                l = num;
            }
            else
        ...
        {
            l = objectInfo.GetObject() is long @int ? @int : Convert.ToInt64(objectInfo.GetObject().ToString());
    ...
    public object BuildOneInstance(NonNativeObjectInfo objectInfo, IOdbCache cache)
    {
        ...
        foreach (var fieldInfo in fields)
        {
            ...
            if (OdbConfiguration.IsLoggingEnabled())
                DLogger.Debug(string.Concat("InstanceBuilder: ", "getting field with name ", fieldInfo.Name,
                    ", attribute id is ",
    ...
    #endregion IInstanceBuilder Members
}