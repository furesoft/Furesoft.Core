...
internal sealed class FileSystemReader : IFileSystemReader
{
	...
	/// </summary>
	/// <remarks>
	///     To hold block number. ODB compute the block number from the oid (as one block has 1000 oids), then it has to search
	///     the position of the block number! This cache is used to keep track of the positions of the block positions The key
	///     is the block number(Long) and the value the position (Long)
	/// </remarks>
    ...
    public long ReadOidPosition(OID oid)
    {
        ...
        if (OdbConfiguration.IsLoggingEnabled())
        {
            ...
            DLogger.Debug(string.Format("ObjectReader: End of readOidPosition for oid {0} returning position ", oid) +
                          positionAsString);
        }
    ...
    private long GetIdBlockPositionFromNumber(long blockNumberToFind)
    {
        ...
        _blockPositions.TryGetValue(blockNumberToFind, out var lposition);
        if (lposition != 0)
            return lposition;
        long currentBlockPosition = StorageEngineConstant.DatabaseHeaderFirstIdBlockPosition;
        while (currentBlockPosition != -1)
