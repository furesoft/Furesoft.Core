...
internal sealed class ObjectReader : IObjectReader
{
    ...
    private ClassInfo ReadClassInfoBody(ClassInfo classInfo)
    {
        if (OdbConfiguration.IsLoggingEnabled())
        {
            ...
            DLogger.Debug(OdbString.DepthToSpaces(_currentDepth) + "ObjectReader: Reading new Class info Body at " +
                          attributesDefinitionPositionAsString);
        }
    ...
    /// <summary>
    ///     Reads an object info(Object meta information like its type and its values) from the database file <p />
    ///     <pre>reads its type and then read all its attributes.</pre>
    /// </summary>
    /// <remarks>
    ///     Reads an object info(Object meta information like its type and its values) from the database file <p />
    ///     <pre>
    ...
    ///     The list of the attribute name for which we need to return a value, an attributename can
    ///     contain relation like profile.name
    /// </param>
    /// <param name="relationAttributeNames">
    ///     The original names of attributes to read the values, an attributename can contain
    ///     relation like profile.name
    /// </param>
    ...
    ///     The type of attribute declared in the ClassInfo. May be different from actual
    ///     attribute type in caso of OID and OdbObjectId
    /// </param>
    ...
    private ArrayObjectInfo ReadArray(long position, bool useCache, bool returnObjects)
    {
        ...
        if (OdbConfiguration.IsLoggingEnabled())
        {
            ...
            DLogger.Debug(OdbString.DepthToSpaces(_currentDepth) + "ObjectReader: reading an array of " +
                          realArrayComponentClassName + " with " +
                          size + " elements");
        }
        ...
        for (var i = 0; i < arraySize; i++)
            try
            {
                if (objectIdentifications[i] != StorageEngineConstant.NullObjectIdId)
                {
                    ...
                    if (!(o is NonNativeDeletedObjectInfo)) array.SetValue(o, i);
                }
                else
                {
            ...
            catch (Exception e)
            {
                var positionAsString = position.ToString();
                throw new OdbRuntimeException(
                    NDatabaseError.InternalError.AddParameter("in ObjectReader.readArray - at position " +
                                                              positionAsString), e);
            }
    ...
    public void LoadMetaModel(IMetaModel metaModel, bool full)
    {
        ...
        for (var i = 0; i < nbClasses; i++)
        {
            ...
            if (OdbConfiguration.IsLoggingEnabled())
                DLogger.Debug(string.Format(
                    "{0}ObjectReader: Reading class header for {1} - oid = {2} prevOid={3} - nextOid={4}",
                    OdbString.DepthToSpaces(_currentDepth),
        ...
        foreach (var currentClassInfo in allClasses)
        {
            ...
            if (OdbConfiguration.IsLoggingEnabled())
                DLogger.Debug(OdbString.DepthToSpaces(_currentDepth) + "ObjectReader:  class body for " +
        ...
        {
            if (OdbConfiguration.IsLoggingEnabled())
                DLogger.Debug(string.Format("{0}ObjectReader: Reading class info last instance {1}",
                    OdbString.DepthToSpaces(_currentDepth),
        ...
        foreach (var actualClassInfo in allClasses)
        {
            ...
            if (OdbConfiguration.IsLoggingEnabled())
            {
                ...
                DLogger.Debug(
                    string.Format("{0}ObjectReader: Reading indexes for {1} : ", OdbString.DepthToSpaces(_currentDepth),
                        actualClassInfo.FullClassName) +
                    count + " indexes");
            }
        ...
        bool useCache, bool returnInstance)
    {
        ...
        if (OdbConfiguration.IsLoggingEnabled())
            DLogger.Debug(OdbString.DepthToSpaces(_currentDepth) +
                          "ObjectReader: Reading Non Native Object info with oid " + oid);
        ...
        if (OdbConfiguration.IsLoggingEnabled())
        {
            ...
            DLogger.Debug(OdbString.DepthToSpaces(_currentDepth) + "ObjectReader: Reading Non Native Object info of " +
                          (classInfo == null
                              ? "?"
                              : classInfo.FullClassName) + " at " +
                          positionAsString + " with id " + oid);
            DLogger.Debug(OdbString.DepthToSpaces(_currentDepth) + "ObjectReader: Object Header is " +
                          objectInfoHeader);
        }
    ...
    /// <returns>
    ///     The object position, if object has been marked as deleted then return
    ///     StorageEngineConstant.DELETED_OBJECT_POSITION @
    /// </returns>
    ...
    #endregion IObjectReader Members
}