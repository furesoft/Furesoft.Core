...
internal sealed class LazyOdbBtreePersister : IBTreePersister, ICommitListener
{
    ...
    private void Persist()
    {
        ...
        if (OdbConfiguration.IsLoggingEnabled())
        {
            ...
            DLogger.Debug(string.Concat("LazyOdbBtreePersister: ", "persist ", _nbPersist.ToString(),
                "  : Saving " + count + " objects - ",
        ...
        while (iterator.MoveNext())
        {
            ...
            if (OdbConfiguration.IsLoggingEnabled())
                DLogger.Debug(string.Concat("LazyOdbBtreePersister: ", "Committing oid " + oid, " | ", i.ToString(),
                    "/", size.ToString(),
                    " | ", (t1 - t0).ToString(), " ms"));
        ...
        if (OdbConfiguration.IsLoggingEnabled())
            DLogger.Debug(string.Concat("LazyOdbBtreePersister: ", nbCommited.ToString(), " commits / ",
                size.ToString()));
    }
    ...
    public void SaveBTree(IBTree treeToSave)
    {
        try
        {
            ...
            if (oid == null)
            {
                ...
                if (OdbConfiguration.IsLoggingEnabled())
                    DLogger.Debug(string.Format("LazyOdbBtreePersister: Saved btree {0} with id {1} and  root {2}",
                        treeToSave.GetId(), oid,
                        treeToSave.GetRoot()));
    ...
    #endregion ICommitListener Members
}