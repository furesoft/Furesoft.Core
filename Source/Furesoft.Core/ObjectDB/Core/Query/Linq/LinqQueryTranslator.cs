...
internal class LinqQueryTranslator : ExpressionTransformer
{
    ...
    private bool Optimize { get; set; } = true;
    ...
    {
        return !(constant.Value is ILinqQueryable queryable)
            ? constant
    ...
    {
        if (!funcMappers.TryGetValue(type, out var mapper))
            mapper = t => t;
    ...
    private static Type MapQueryableToEnumerable(Type type)
    {
        ...
        else if (type == typeof(IQueryable)) type = typeof(IEnumerable);
