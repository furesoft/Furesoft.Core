...
internal abstract class GenericQueryExecutor : IMultiClassQueryExecutor
{
    ...
    /// <summary>
    ///     Check if the object with oid matches the query, returns true This method must compute the next object oid and the
    ///     orderBy key if it exists!
    /// </summary>
    /// <param name="oid"> The object position </param>
    /// <param name="loadObjectInfo">
    ///     To indicate if object must loaded (when the query indicator 'in memory' is false, we do
    ///     not need to load object, only ids)
    /// </param>
        ...
        IMatchingObjectAction queryResultAction)
    {
        ...
        if (OdbConfiguration.IsLoggingEnabled())
            DLogger.Debug(string.Format("GenericQueryExecutor: loading {0} instance(s) of {1}", nbObjects,
    ...
    {
        if (OdbConfiguration.IsLoggingEnabled())
            DLogger.Debug(string.Format("GenericQueryExecutor: loading Object with oid {0} - class {1}",
                Query.GetOidOfObjectToQuery(),
                ClassInfo.FullClassName));
    ...
    {
        return @object is AttributeValuesMap attributeValuesMap
    ...
    #endregion IMultiClassQueryExecutor Members
}