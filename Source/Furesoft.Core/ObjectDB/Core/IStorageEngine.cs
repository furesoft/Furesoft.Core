...
/// </summary>
public interface IStorageEngine : ITriggersEngine, IClassInfoProvider, IQueryEngine
{
    ...
    /// </summary>
    /// <remarks>
    ///     Store an object in an database. To detect if object must be updated or insert, we use the cache. To update an
    ///     object, it must be first selected from the database. When an object is to be stored, if it exist in the cache, then
    ///     it will be updated, else it will be inserted as a new object. If the object is null, the cache will be used to
    ///     check if the meta representation is in the cache
    /// </remarks>
