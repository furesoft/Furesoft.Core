...
/// </summary>
/// <remarks>
///     Manage Object info differences. compares two object info and tells which objects in the object hierarchy has
///     changed. This is used by the update to process to optimize it and actually update what has changed
/// </remarks>
internal sealed class ObjectInfoComparator : IObjectInfoComparator
{
    ...
    private bool HasChanged(NonNativeObjectInfo nnoi1, NonNativeObjectInfo nnoi2, int objectRecursionLevel)
    {
        ...
        _alreadyCheckingObjects.TryGetValue(nnoi2, out var n);
        if (n != 0)
            return false;
        ...
        for (var id = 1; id <= nnoi1.GetMaxNbattributes(); id++)
        {
            ...
            {
                if (value2 is NativeObjectInfo nativeObjectInfo)
                {
                    StoreChangedObject(nnoi1, nnoi2, id, objectRecursionLevel);
                    _nbChanges++;
                }

                if (value2 is ObjectReference objectReference)
                {
            ...
            if (value1.IsEnumObject())
            {
                ...
                var enumHasChanged =
                    !enoi1.GetEnumClassInfo().ClassInfoId.Equals(enoi2.GetEnumClassInfo().ClassInfoId) ||
    ...
    #endregion IObjectInfoComparator Members
}