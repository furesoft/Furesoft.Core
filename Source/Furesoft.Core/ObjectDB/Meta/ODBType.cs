...
public sealed class OdbType
{
    ...
    /// </summary>
    public OdbType SubType { get; set; }
    ...
    {
        return new(_isPrimitive, Id, name, Size) {SubType = SubType};
    }
    ...
    public static OdbType GetFromClass(Type clazz)
    {
        ...
        if (clazz.IsArray)
        {
            var type = new OdbType(Array._isPrimitive, ArrayId, Array.Name, 0)
                {SubType = GetFromClass(clazz.GetElementType())};
    ...
    {
        return Id == IntegerId && type2.Id == IntegerId;
    }
