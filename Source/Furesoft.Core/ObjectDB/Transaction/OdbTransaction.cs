...
internal sealed class OdbTransaction : ITransaction
{
	...
	/// <summary>
	///     To indicate if all write actions are in memory - if not, transaction must read them from transaction file o commit
	///     the transaction
	/// </summary>
    ...
    private void AddWriteAction(WriteAction writeAction, bool persistWriteAction)
    {
        if (OdbConfiguration.IsLoggingEnabled())
            DLogger.Info(string.Format("OdbTransaction: Adding WriteAction in Transaction of session {0}",
                _session.GetId()));
    ...
    /// </summary>
    /// <remarks>
    ///     Set the write position (position in main database file). This is used to know if the next write can be appended to
    ///     the previous one (in the same current Write Action) or not.
    /// </remarks>
    ...
    public void SetWritePosition(long position)
    {
        if (position != _currentWritePositionInWa)
        {
            ...
            if (_currentWriteAction != null) AddWriteAction(_currentWriteAction, true);
    ...
    public void ManageWriteAction(long position, byte[] bytes)
    {
        ...
        {
            if (_currentWriteAction == null) _currentWriteAction = new(position);
