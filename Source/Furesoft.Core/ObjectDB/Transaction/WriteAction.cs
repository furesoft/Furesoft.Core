...
/// <summary>
///     The WriteAction class is the description of a Write operation that will be applied to the main database file when
///     committing.
/// </summary>
/// <remarks>
///     The WriteAction class is the description of a Write operation that will be applied to the main database file when
    ...
    internal static WriteAction Read(IFileSystemInterface fsi)
    {
        try
        {
            ...
            if (OdbConfiguration.IsLoggingEnabled())
                DLogger.Debug(string.Format("Transaction WriteAction: Loading Write Action at {0} => {1}",
                    fsi.GetPosition(), writeAction));

            return writeAction;
        }
        catch (OdbRuntimeException)
        {
            DLogger.Error(string.Format("Transaction WriteAction: error reading write action at position {0}",
                fsi.GetPosition()));
            throw;
        }
    }
